 ...
 * @returns {any}
 */
export function inferDataset(files, record_sets, options) {
 ...
 * @returns {any}
 */
export function inferFiles(files, options) {
 ...
 * @returns {any}
 */
export function inferRecords(record_sets) {
 ...
 * @returns {string}
 */
export function to_spec(spec_json) {
    let deferred3_0;
    let deferred3_1;
    try {
 ...
 * @param {string} spec_json
 */
export function validate(spec_json) {
    const ptr0 = passStringToWasm0(spec_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('nirs4all_io_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
