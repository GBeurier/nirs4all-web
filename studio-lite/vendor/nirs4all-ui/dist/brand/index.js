...
const BRAND_ID_SET = new Set(NIRS4ALL_BRANDS.map((brand) => brand.id));
export function isNirs4allBrandId(value) {
    return BRAND_ID_SET.has(value);
}
export function getNirs4allBrandDefinition(id) {
...
export function listNirs4allBrands() {
    return NIRS4ALL_BRANDS;
}
export function getNirs4allBrandAssetPath(brand, variant) {
    return resolveBrand(brand).assets[variant];
}
export function generateNirs4allBrandSvg(brand, options = {}) {
...
function buildIconSvg(brand, options) {
...
function buildHorizontalSvg(brand, options) {
    const title = escapeXml(options.title ?? brand.name);
...
function buildStackedSvg(brand, options) {
    const title = escapeXml(options.title ?? brand.name);
...
function waveAnimation(targetId) {
