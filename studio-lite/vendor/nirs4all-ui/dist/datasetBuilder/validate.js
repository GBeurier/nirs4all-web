...
export function validateBuilder(sources, locale = "fr") {
    const t = COPY[locale];
    const checks = [];
    if (sources.length === 0) {
        return {
    ...
    if (schema.xSources.length)
        assignedRoles.add("x");
    if (schema.yColumns.length)
        assignedRoles.add("y");
    if (schema.idColumns.length)
        assignedRoles.add("id");
    if (schema.metadataColumns.length)
        assignedRoles.add("metadata");
    if (schema.partitionColumns.length)
        assignedRoles.add("partition");
    if (schema.replicateColumns.length)
        assignedRoles.add("replicate");
    checks.push({
        id: "roles-assigned",
    ...
    // X presence.
    checks.push(schema.xSources.length > 0
    ...
    const hasTrainOnlyX = sources.some((s) => s.usage.useAs === "x_test" || s.usage.useAs === "y_test");
    checks.push(schema.yColumns.length > 0
    ...
    // IDs.
    checks.push(schema.idColumns.length > 0
    ...
    const unassignedNumeric = sources.reduce((acc, s) => acc +
        s.columns.filter((c) => c.assignedRole === "ignored" && NUMERIC.has(c.detectedType)).length, 0);
    ...
    const status = checks.reduce((acc, c) => worst(acc, c.level), "ok");
    if (status === "ok") {
