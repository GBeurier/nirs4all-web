...
export function isSpectralHeader(name) {
    const n = normalize(name);
    if (/^(wavelength|wl|wave|band|nm|w)_?\d+(\.\d+)?$/.test(n))
        return true;
    if (/^\d+(\.\d+)?_?nm$/.test(n))
        return true;
    if (/^\d+(\.\d+)?$/.test(n))
...
export function guessSignalType(columns, fileType) {
    const ft = fileType.toLowerCase();
    if (ft.includes("image") || ft === "folder")
        return "image";
 ...
 */
export function detectColumnRole(column, spectralRun) {
    ...
    if (matchesAny(name, ID_NAMES))
        return { assignedRole: "id", semanticType: "identifier" };
    if (matchesAny(name, REPLICATE_NAMES))
        return { assignedRole: "replicate" };
    if (matchesAny(name, PARTITION_NAMES))
        return { assignedRole: "partition" };
    if (matchesAny(name, GROUP_NAMES))
        return { assignedRole: "metadata" };
    if (matchesAny(name, Y_NAMES)) {
        const task = NUMERIC_TYPES.has(detectedType) ? "regression" : "classification";
 ...
 */
function spectralRunFlags(columns) {
...
export function autoDetectColumns(columns) {
    ...
    return columns.map((col, i) => {
        ...
        const next = { ...col, assignedRole: detected.assignedRole };
        if (detected.semanticType !== undefined)
            next.semanticType = detected.semanticType;
        else
...
export function autoDetectSource(source) {
