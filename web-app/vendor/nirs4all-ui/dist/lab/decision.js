 ...
 */
export function buildDecisionView(input, thresholds, locale = 'fr') {
    ...
    if (input.gateRejected === true) {
        return view('out_of_domain', 'measurement_artifact', 'low', { fr: 'Domaine non admissible (gate OOD/SSI négatif)', en: 'Domain not admissible (OOD/SSI gate rejected)' }, locale);
    }
    if (ad !== undefined && ad >= t.adReject) {
        return view('out_of_domain', 'out_of_domain', 'low', { fr: 'Spectre hors du domaine du modèle actuel', en: 'Spectrum outside the current model domain' }, locale);
    }
    if (width !== undefined && width >= t.intervalMax) {
        return view('out_of_domain', 'uncertain_prediction', 'low', { fr: 'Intervalle de prédiction trop large pour être utilisable', en: 'Prediction interval too wide to be usable' }, locale);
    }
    if (input.strongOutlier === true) {
        ...
        return view('out_of_domain', 'measurement_artifact', 'low', { fr: 'Outlier spectral fort — à vérifier avant tout usage', en: 'Strong spectral outlier — verify before any use' }, locale);
    }
    // --- 🟠 caution: on the border / uncertain -------------------------------
    if (ad !== undefined && ad >= t.adWarn) {
        return view('caution', 'near_border', 'medium', { fr: 'En bordure du domaine connu du modèle', en: 'At the edge of the model’s known domain' }, locale);
    }
    if (width !== undefined && width >= t.intervalWarn) {
        return view('caution', 'uncertain_prediction', 'medium', { fr: 'Incertitude élevée (intervalle large)', en: 'High uncertainty (wide interval)' }, locale);
    }
    if (input.extrapolation === true) {
        return view('caution', 'uncertain_prediction', 'medium', { fr: "Valeur prédite proche ou hors de la gamme observée à l'apprentissage", en: 'Predicted value near or outside the training range' }, locale);
    }
    // --- 🔵 informative for improvement (clean but rare) ---------------------
    if (density !== undefined && density <= t.lowDensity) {
        return view('informative', 'enrichment_candidate', 'high', { fr: 'Échantillon rare/peu couvert — le mesurer enrichirait le modèle', en: 'Rare / sparsely-covered sample — measuring it would enrich the model' }, locale);
    }
    // --- 🟢 reliable ----------------------------------------------------------
    return view('reliable', 'in_domain', 'high', { fr: 'Dans le domaine connu, intervalle raisonnable, pas d’extrapolation', en: 'In the known domain, reasonable interval, no extrapolation' }, locale);
}
function view(color, category, confidence, reason, locale) {
...
export function isDecisionColor(value) {
