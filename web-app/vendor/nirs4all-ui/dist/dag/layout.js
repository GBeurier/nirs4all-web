...
function topoOrder(n, out, inDeg) {
...
export function layoutDag(eff, opts) {
    const { direction } = opts;
    const n = eff.nodes.length;
    if (n === 0)
        return emptyLayout(direction);
    const m = metricsFor(direction);
    const index = new Map();
    ...
    const topoRank = new Array(n).fill(0);
    order.forEach((node, rank) => (topoRank[node] = rank));
    const layer = new Array(n).fill(0);
    for (const u of order) {
        for (const v of out[u]) {
            if (topoRank[u] < topoRank[v] && layer[u] + 1 > layer[v]) {
                layer[v] = layer[u] + 1;
            }
        }
    }
    ...
    const reindex = () => {
        for (const layerNodes of layers)
            layerNodes.forEach((node, i) => (pos[node] = i));
    };
    reindex();
    // --- 2. barycenter crossing reduction (alternating down / up sweeps)
    const barycenter = (node, neighbors, onLayer) => {
        ...
        return count === 0 ? pos[node] : sum / count;
    };
    for (let sweep = 0; sweep < SWEEPS; sweep += 1) {
        for (let L = 1; L <= maxLayer; L += 1) {
            ...
            for (const node of layerNodes)
                bc.set(node, barycenter(node, inn, L - 1));
            layerNodes.sort((a, b) => bc.get(a) - bc.get(b));
        }
        reindex();
        for (let L = maxLayer - 1; L >= 0; L -= 1) {
            ...
            const bc = new Map();
            for (const node of layerNodes)
                bc.set(node, barycenter(node, out, L + 1));
            layerNodes.sort((a, b) => bc.get(a) - bc.get(b));
        }
        reindex();
    }
    ...
    let globalAlong = 0;
    for (const layerNodes of layers)
        globalAlong = Math.max(globalAlong, layerNodes.length * m.alongStep);
    const layoutNodes = new Array(n);
    for (let L = 0; L <= maxLayer; L += 1) {
        ...
        layerNodes.forEach((node, o) => {
            ...
            layoutNodes[node] = { node: eff.nodes[node], x, y, w: m.nodeW, h: m.nodeH, layer: L, order: o };
        });
    }
    ...
    for (const edge of eff.edges) {
        ...
        if (s === undefined || t === undefined)
            continue;
        const a = layoutNodes[s];
        const b = layoutNodes[t];
...
function edgeRoute(edge, a, b, direction, back) {
    let sx;
    let sy;
    let tx;
    let ty;
...
function buildFrames(layoutNodes, hierarchy) {
    if (!hierarchy || hierarchy.groups.size === 0)
        return [];
    const boxes = new Map();
    const extend = (id, ln) => {
    ...
    for (const id of boxes.keys())
        maxDepth = Math.max(maxDepth, hierarchy.groups.get(id)?.depth ?? 0);
    const frames = [];
