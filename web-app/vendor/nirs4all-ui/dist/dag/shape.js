...
export const SHAPE_CHANGE_STYLE = {
    none: { glyph: "", tone: "" },
...
export function shapeChange(inputs, out) {
    ...
    if (out.representation === "prediction" && !inAllPred)
        return "predict";
    if (ins.length > 1 || (out.features != null && inSrc > outSrc && outSrc >= 1))
        return "join";
    if (inSamples != null && out.samples != null) {
        if (out.samples > inSamples)
            return "rows-up";
        if (out.samples < inSamples)
            return "rows-down";
    }
    if (inFeat != null && out.features != null) {
        if (out.features > inFeat)
            return "feat-up";
        if (out.features < inFeat)
            return "feat-down";
    }
...
export function describeShapeDelta(inputs, out) {
    ...
    switch (kind) {
        ...
        case "rows-down": {
            if (inSamples == null || out.samples == null)
                return kind === "rows-up" ? "more rows" : "fewer rows";
        ...
        case "join": {
            const parts = [];
            if (inFeat != null && out.features != null)
                parts.push(`features ${formatCount(inFeat)} → ${formatCount(out.features)}`);
            const inSrc = ins.reduce((a, s) => a + sourceCount(s), 0);
            const outSrc = sourceCount(out);
            if (inSrc !== outSrc)
                parts.push(`${inSrc} → ${outSrc} sources`);
...
function joinInputs(inputs, isPrediction) {
    ...
    for (const s of inputs) {
        ...
        if (s.sources)
            ...
            sources.push({ name: s.representation ?? "source", features: s.features, ...(s.representation ? { kind: s.representation } : {}) });
    }
    const out = { representation: "tabular_numeric" };
    if (samples != null)
        out.samples = samples;
    if (anyFeat)
    ...
    if (sources.length > 0)
        out.sources = sources;
    if (inputs.length > 1)
        out.note = isPrediction ? "stacked predictions" : "joined sources";
    return out;
}
const defaultRule = ({ node, category, inputs, entry }) => {
    const first = inputs[0];
    switch (category) {
        case "data":
            if (node.kind === "source_join" || inputs.length > 1)
                return joinInputs(inputs, false);
            return entry ?? first;
        case "merge":
        ...
        case "model": {
            ...
            const out = { targets, representation: "prediction" };
 ...
 */
export function deriveShapes(graph, options = {}) {
    ...
    const order = graph.nodes.filter((n) => deg.get(n.id) === 0).map((n) => n.id);
    for (let h = 0; h < order.length; h += 1) {
        for (const v of succ.get(order[h])) {
    ...
    const outShape = new Map();
    const inShapes = new Map();
    for (const id of order) {
        const node = byId.get(id);
        const ins = preds.get(id).map((p) => outShape.get(p)).filter((s) => s != null);
        inShapes.set(id, ins);
        if (node.io?.out) {
