...
export function orientedValue(score, lowerIsBetter) {
...
export function dedupeOrderedTokens(steps) {
    const seen = new Set();
    const out = [];
 ...
 */
export function computeChainPoints(chains, metric, lens) {
    const oriented = chains.map((chain) => orientedValue(chain.score, metric.lowerIsBetter));
    const goodness = new Array(chains.length);
    if (lens === "raw") {
        for (let i = 0; i < chains.length; i += 1)
            goodness[i] = oriented[i];
    }
    else {
