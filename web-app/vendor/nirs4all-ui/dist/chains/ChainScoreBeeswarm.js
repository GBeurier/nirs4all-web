 ...
 */
export function ChainScoreBeeswarm({ analysis, focusToken, width = 460, height = 220, padding, maxPoints, withColor = N4_VIZ_COLORS.teal, withoutColor = N4_VIZ_COLORS.slate, title = "Distribution with vs without", className, }) {
    ...
    const domain = niceExtent([analysis.goodnessExtent.min, analysis.goodnessExtent.max, analysis.baseline], 0.04);
    ...
    return (_jsxs("svg", { className: cx("n4chains", "n4chains-beeswarm", className), viewBox: `0 0 ${width} ${height}`, role: "img", "aria-label": title, preserveAspectRatio: "xMidYMid meet", children: [_jsx("title", { children: title }), _jsx("text", { className: "n4chains-title", x: 8, y: 16, textAnchor: "start", children: title }), _jsxs("text", { className: "n4chains-caption", x: round(plotRight), y: 16, textAnchor: "end", children: [CHAIN_LENS_LABELS[analysis.lens], " \u00B7 higher = better"] }), xTicks.map((t) => (_jsx("line", { className: "n4chains-grid", x1: round(xScale(t)), x2: round(xScale(t)), y1: top, y2: bottom }, `gx-${t}`))), _jsx("line", { className: "n4chains-baseline", x1: baselineX, x2: baselineX, y1: top - 2, y2: bottom }), lanes.map((lane) => {
                const laneMid = lane.y0 + laneH / 2;
                const hasIqr = Number.isFinite(lane.stat.q1) && Number.isFinite(lane.stat.q3);
                return (_jsxs("g", { className: "n4chains-lane", "data-lane": lane.key, children: [_jsx("text", { className: "n4chains-lane-label", x: left - 8, y: round(laneMid - 4), textAnchor: "end", children: lane.label }), _jsxs("text", { className: "n4chains-lane-readout", x: left - 8, y: round(laneMid + 9), textAnchor: "end", children: ["n=", lane.stat.n, " \u00B7 med ", fmt(lane.stat.median)] }), hasIqr ? (_jsx("rect", { className: "n4chains-lane-iqr", x: round(xScale(lane.stat.q1)), y: round(laneMid - dotBand / 2), width: round(Math.max(0, xScale(lane.stat.q3) - xScale(lane.stat.q1))), height: round(dotBand), fill: lane.color })) : null, lane.points.map((point) => (_jsx("circle", { className: "n4chains-dot", cx: round(xScale(point.goodness)), cy: round(laneMid - dotBand / 2 + jitter(point.id) * dotBand), r: 2.2, fill: lane.color }, point.id))), Number.isFinite(lane.stat.median) ? (_jsx("line", { className: "n4chains-lane-median", x1: round(xScale(lane.stat.median)), x2: round(xScale(lane.stat.median)), y1: round(laneMid - dotBand / 2 - 4), y2: round(laneMid + dotBand / 2 + 4), stroke: lane.color })) : null] }, lane.key));
            }), _jsx("line", { className: "n4chains-axis", x1: left, x2: plotRight, y1: round(top + laneH), y2: round(top + laneH) }), _jsx("line", { className: "n4chains-axis", x1: left, x2: plotRight, y1: bottom, y2: bottom }), xTicks.map((t) => (_jsx("text", { className: "n4chains-tick", x: round(xScale(t)), y: bottom + 14, textAnchor: "middle", children: fmt(t) }, `tx-${t}`)))] }));
}
//# sourceMappingURL=ChainScoreBeeswarm.js.map