...
export const REGRESSION_METRICS = ["r2", "rmse", "rpd"];
...
// ============================================================================
export const ALL_GENERAL_METRICS = [];
export const ALL_REGRESSION_METRICS = [
...
export const ALL_CLASSIFICATION_METRICS = [
...
export const ALL_SCORE_METRICS = [
    ...ALL_GENERAL_METRICS,
    ...ALL_REGRESSION_METRICS,
    ...ALL_CLASSIFICATION_METRICS,
];
const METRIC_DEFINITIONS_BY_KEY = new Map(ALL_SCORE_METRICS.map(metric => [metric.key, metric]));
export function isClassificationTaskType(taskType) {
    return CLASSIFICATION_TASK_TYPES.has((taskType || "").toLowerCase());
}
function hasRegressionTaskType(taskType) {
    return !!taskType && !isClassificationTaskType(taskType);
}
export function getMetricsForTaskType(taskType) {
...
export function orderMetricKeys(metricKeys) {
...
export function getMetricDefinitions(metricKeys) {
    return orderMetricKeys(metricKeys)
        .map(key => METRIC_DEFINITIONS_BY_KEY.get(key))
        .filter((metric) => !!metric);
}
export function groupMetricDefinitions(metricKeys) {
    const labels = {
        general: "General",
    ...
    return ["regression", "multiclass", "binary", "general"]
        .map(group => ({
        group,
        label: labels[group],
...
function combineMetricSelections(...metricLists) {
    return orderMetricKeys(metricLists.flatMap(metrics => metrics ?? []));
}
export function getDefaultSelectedMetricsForTaskTypes(taskTypes) {
    let hasClassification = false;
    let hasRegression = false;
    for (const taskType of taskTypes) {
        if (isClassificationTaskType(taskType))
            hasClassification = true;
        else if (hasRegressionTaskType(taskType))
            hasRegression = true;
    }
    if (hasClassification && hasRegression) {
        return combineMetricSelections(DEFAULT_DATASET_ITEM_REGRESSION_METRICS, DEFAULT_DATASET_ITEM_CLASSIFICATION_METRICS);
    }
    if (hasClassification)
        return [...DEFAULT_DATASET_ITEM_CLASSIFICATION_METRICS];
    return [...DEFAULT_DATASET_ITEM_REGRESSION_METRICS];
}
export function getLegacySelectedMetricsForTaskTypes(taskTypes) {
    let hasClassification = false;
    let hasRegression = false;
    for (const taskType of taskTypes) {
        if (isClassificationTaskType(taskType))
    ...
    if (hasClassification && hasRegression) {
        return combineMetricSelections(LEGACY_DATASET_ITEM_REGRESSION_METRICS, LEGACY_DATASET_ITEM_CLASSIFICATION_METRICS);
    }
    if (hasClassification)
        return [...LEGACY_DATASET_ITEM_CLASSIFICATION_METRICS];
    return [...LEGACY_DATASET_ITEM_REGRESSION_METRICS];
}
export function getDefaultSelectionUpgradeCandidatesForTaskTypes(taskTypes) {
    let hasClassification = false;
    let hasRegression = false;
    for (const taskType of taskTypes) {
        if (isClassificationTaskType(taskType))
...
export function getAvailableMetricKeysForTaskTypes(taskTypes) {
    let hasClassification = false;
    let hasRegression = false;
    for (const taskType of taskTypes) {
        if (isClassificationTaskType(taskType))
...
export function filterMetricsForTaskType(metricKeys, taskType) {
...
export function getAvailableMetrics(taskType) {
    if (isClassificationTaskType(taskType)) {
...
export function getPresetsForTaskType(taskType) {
    if (isClassificationTaskType(taskType)) {
        return CLASSIFICATION_PRESETS;
    }
    return REGRESSION_PRESETS;
}
export function getPresetsForTaskTypes(taskTypes) {
    let hasClassification = false;
    let hasRegression = false;
    for (const taskType of taskTypes) {
        if (isClassificationTaskType(taskType))
    ...
    return [
        {
            id: "essential",
            label: "Essential",
            keys: combineMetricSelections(REGRESSION_PRESETS.find(preset => preset.id === "essential")?.keys, classificationEssential),
        },
        {
            id: "nirs",
            label: "NIRS",
            keys: combineMetricSelections(REGRESSION_PRESETS.find(preset => preset.id === "nirs")?.keys, classificationEssential),
        },
        {
            id: "ml",
            label: "ML",
            keys: combineMetricSelections(REGRESSION_PRESETS.find(preset => preset.id === "ml")?.keys, classificationEssential),
        },
        {
            id: "full",
            label: "Full",
            keys: combineMetricSelections(REGRESSION_PRESETS.find(preset => preset.id === "full")?.keys, classificationFull),
        },
    ];
}
/** Get the default selected metrics for a task type. */
export function getDefaultSelectedMetrics(taskType) {
    if (isClassificationTaskType(taskType)) {
        return [...DEFAULT_DATASET_ITEM_CLASSIFICATION_METRICS];
    }
    if (taskType == null) {
...
//# sourceMappingURL=scoreMetricCatalog.js.map