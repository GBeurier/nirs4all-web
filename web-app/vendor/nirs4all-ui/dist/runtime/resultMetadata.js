...
function readRecordField(source, keys) {
    if (!source)
        return null;
    for (const key of keys) {
...
function readArrayField(source, keys) {
    if (!source)
        return null;
    for (const key of keys) {
        const value = source[key];
...
function runtimeRecordCandidates(source) {
...
function normalizeDiagnosticMessage(raw, record) {
...
function resolveDiagnosticTone(record) {
...
function normalizeRuntimeDiagnostic(raw, index) {
...
function collectRuntimeDiagnostics(source) {
...
export function normalizeRuntimeDiagnostics(source) {
    return collectRuntimeDiagnostics(source)
        .map(normalizeRuntimeDiagnostic)
        .filter((item) => item != null);
}
export function formatRuntimeTokenLabel(value) {
    if (!value)
        return "";
    const normalized = value.trim();
    const lower = normalized.toLowerCase();
    if (lower === "dag-ml" || lower === "dag_ml" || lower === "dagml")
        return "DAG-ML";
    if (lower === "wasm" || lower === "wasm-local" || lower === "wasm_local")
        return lower.includes("local") ? "Local WASM" : "WASM";
    if (lower === "n4a")
        return "N4A";
    return normalized
        .split(/[-_\s]+/)
...
function resolveRuntimeEngineTone(engine, isFallback) {
    ...
    if (normalized === "dag-ml" || normalized === "dag_ml" || normalized === "dagml")
        return "success";
    if (normalized === "legacy")
        return "muted";
    return "default";
}
function normalizeRuntimeEngineToken(value) {
    return value.trim().toLowerCase().replace(/_/g, "-");
}
export function buildRuntimeEngineStatus(source) {
    const candidates = runtimeRecordCandidates(source);
    ...
    const isFallback = Boolean(engine
        && requestedEngine
...
export function buildRuntimeNativeResultsAffordance(input = {}) {
