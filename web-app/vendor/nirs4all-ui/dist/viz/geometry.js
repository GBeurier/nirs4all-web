...
export function buildFrame(width, height, padding = {}) {
    const resolved = {
        top: padding.top ?? 16,
...
export function niceExtent(values, padFraction = 0.05) {
...
export function sharedExtent(...lists) {
 ...
 */
export function makeScale(domain, rangeMin, rangeMax) {
    const span = domain.max - domain.min || 1;
    const slope = (rangeMax - rangeMin) / span;
    return (value) => rangeMin + (value - domain.min) * slope;
}
/** Evenly spaced, human-friendly tick values across an extent. */
export function ticks(extent, count = 5) {
