...
export function deriveSchema(sources) {
    const summary = {
        xSources: [],
        yColumns: [],
        metadataColumns: [],
    ...
    for (const source of sources) {
        ...
        if (hasX)
            summary.xSources.push(source.id);
    }
    return summary;
}
...
function defaultBuckets(mode, total) {
    let raw;
    if (mode === "train_val_test") {
        ...
        const k = 5;
        raw = Array.from({ length: k }, (_, i) => [`fold_${i + 1}`, `Fold ${i + 1}`, total / k]);
    }
    else {
        raw = [
...
export function findPartitionColumn(sources) {
    for (const source of sources) {
        const col = source.columns.find((c) => c.assignedRole === "partition");
        if (col)
            return ref(source, col);
    }
    return undefined;
}
/** Immutably set the role for a set of column ids inside one source. */
export function assignRoleToColumns(source, columnIds, role) {
    return {
        ...source,
        columns: source.columns.map((col) => columnIds.has(col.id) ? { ...col, assignedRole: role, manual: true } : col),
    };
}
/** Immutably toggle a column's selection flag. */
export function toggleColumnSelection(source, columnId, selected) {
    return {
        ...source,
        columns: source.columns.map((col) => col.id === columnId ? { ...col, selected: selected ?? !col.selected } : col),
    };
}
export function isRecognizedPartitionValue(value) {
